...
<html>
<head>
	<title>Maps Bencana - Verivikasi User</title>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		<div class="row">
			<ol class="breadcrumb">
				...
				<li class="active">List Verifikasi Akun</li>
			</ol>
		</div><!--/.row-->
		
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">List Verifikasi Akun</h1>
			</div>
		</div><!--/.row-->
			...
			<div class="col-md-12">
				<div class="panel panel-default">
					...
					<div class="panel-body" id="map-canvas">
						...
						<?php $query = mysqli_query($con,"SELECT id_username, username, password, status,pengguna.id_pengguna, nama, tempatlahir, tanggallahir, alamat, kelurahan,kecamatan, email, sosialmedia, fotoktp, nomorktp,no_telepon FROM username JOIN pengguna ON username.id_pengguna = pengguna.id_pengguna WHERE status='$noVerifikasi'"); while ($data = mysqli_fetch_array($query))
						{
						$id = $data['id_username'];
						...
						echo ("
							...
							<td><form action='service/function.php?id=$id' method='post'>
								<button id='VerifikasiUser' type='submit' class='btn btn-md btn-warning' name='VerifikasiUser' onclick='verifData($id)'>Verifikasi</button></form></td>
							<td><form action='service/function.php?id=$id' method='post'>
								<button id='HapusVerifikasiUser' type='submit' class='btn btn-md btn-danger' name='HapusVerifikasiUser' value'$id' onclick='deleteData($id)'>Hapus</button></form></td> ");                 
            }
          ?>
