...
<html>
<head>
	<title>Kajian Islam Samarinda - Daftar Pengguna</title>
			...
			<div class="col-md-12">
				<div class="panel panel-default">
					...
					<div class="panel-body" id="map-canvas">
						...
						$query = mysqli_query($con,"SELECT id_username, username, password, status,pengguna.id_pengguna, nama, tempatlahir, tanggallahir, alamat, kelurahan, kecamatan, email, sosialmedia, fotoktp, nomorktp,no_telepon FROM username JOIN pengguna ON username.id_pengguna = pengguna.id_pengguna where status='Verifikasi' && level='User'"); while ($data = mysqli_fetch_array($query))
						{
						$id = $data['id_username'];
						...
						echo ("
							...
							<td><form action='EditUser.php?id=$id' method='post'>
							...
							<td><form action='service/function.php?id=$id' method='post'>
								<button id='BlokirUser' type='submit' class='btn btn-md btn-warning' name='BlokirUser' onclick='blockData($id)'>Blokir</button></form></td>
							<td><form action='service/function.php?id=$id' method='post'>
								<button id='HapusUser' type='submit' class='btn btn-md btn-danger' name='HapusUser' value'$id' onclick='deleteData($id)'>Hapus</button></form></td> ");                 
            }
          ?>
